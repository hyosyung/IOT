	...
	for(ys=0;ys<16;ys++)
	{
		for(xs=0;xs<8;xs++)
		{
			if(temp[ys]&bitmask[xs])
			{
				...
				{
					Lcd_Put_Pixel(x+3*xs,y+3*ys+1,color);
					Lcd_Put_Pixel(x+3*xs+1,y+3*ys,color);
					Lcd_Put_Pixel(x+3*xs,y+3*ys,color);
					Lcd_Put_Pixel(x+3*xs+1,y+3*ys+1,color);
				}
                                else if( (zx==4)&&(zy==4) )
				{
					Lcd_Put_Pixel(x+4*xs,y+4*ys+1,color);
					Lcd_Put_Pixel(x+4*xs+1,y+4*ys,color);
					Lcd_Put_Pixel(x+4*xs,y+4*ys,color);
					Lcd_Put_Pixel(x+4*xs+1,y+4*ys+1,color);
				}
                                else if( (zx==5)&&(zy==5) )
				{
					Lcd_Put_Pixel(x+5*xs,y+9*ys+1,color);
					Lcd_Put_Pixel(x+5*xs+1,y+9*ys,color);
					Lcd_Put_Pixel(x+5*xs,y+9*ys,color);
					Lcd_Put_Pixel(x+5*xs+1,y+9*ys+1,color);
				}
